...
# [mcp-server-cantera](https://github.com/StepFunctionLLC/mcp-server-cantera)
...
- **`src/`**: Source code for the MCP server (includes bundled mechanism files)
...
- `nasa_gas.yaml` — Required for metal combustion and species thermo (gas phase species)
- `nasa_condensed.yaml` — Required for metal combustion (solid/liquid species)
- `JetSurf2.yaml` — Jet fuel surrogate mechanism

To use additional custom mechanism files:

1. Place YAML mechanism files in the `mechanisms/` folder inside the installed package
